...
ItemControllers.controller("ListController", ['$scope','$http', 
	function($scope, $http)
		{    
				$http.get('/json/ItemClassInfoV4.json').success (function(data){
	 ...
	 function($scope, $http, $routeParams)
		{    
				...
				$scope.ItemVariable = data;
					...
					$http.get('/json/CraftShopInfo.json').success (function(data6){
					$scope.CraftShopInfo = data6;
					})
				...
				setTimeout(function(){	
					...
					UsedInExpertiseButton = function(){
							$('.mainitemclassexp').val(ItemClassValue).trigger('input')
					...
					DISQUS.reset({
					  reload: true,
					  config: function () {  
						this.page.identifier = $('.itemdetail').attr('value');  
